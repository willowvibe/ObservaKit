...
def run_profiling(table_name: str, db: Session = Depends(get_db)):
    ...
    for col in schema:
        ...
        is_numeric = any(
            t in col_type for t in ["int", "decimal", "numeric", "float", "real", "double"]
        )
        ...
        try:
            ...
            if results:
                ...
                profile = ColumnProfile(
                    ...
                    mean_value=float(res.get("mean_val"))
                    if res.get("mean_val") is not None
                    else None,
...
def get_latest_profile(table_name: str, db: Session = Depends(get_db)):
    ...
    latest_run = (
        db.query(ColumnProfile.profiled_at)
        .filter(ColumnProfile.table_name == table_name)
    ...
    records = (
        ...
        .filter(ColumnProfile.table_name == table_name, ColumnProfile.profiled_at == latest_run[0])
        .all()
    )
